    ...
    if ( $this->form_validation->run() == FALSE ) {
      ...
      if ($this->$model->insert_entry($atributo)) {
        ...
        if ($estado_archivos['status'] === TRUE) {
          echo json_encode( array( 'status' => 'success', 'msg' => 'Carga exitosa') );
        } else {
          echo json_encode( array( 'status' => 'error', 'msg' => $estado_archivos['error_msg'] ) );
        }
      } else {
        echo json_encode( array( 'status' => 'error', 'msg' => 'No se pudo cargar el atributo' ) );
      }
    } 
  }
